...
class ClockEventHandler(EventHandlerInterface):
    ...
    Once the state is passed to this handler through handle_state(state),
    the in game clock can be retrieved using get_game_clock().
    """
    ...
    def states_are_usable(self, state):
        return self.last_state is not None \
            ...
            and state.get("map") is not None \
            and self.last_state.get("map") is not None
